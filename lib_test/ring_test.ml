...
module Xenstore_frontend = Xenstore_ring.Frontend(Inheap_events)
module Xenstore_backend = Xenstore_ring.Backend(Inheap_events)
...
let with_xenstores fn =
	...
	let channel' = Lwt_main.run(Inheap_events.connect 0 port) in
        let f = Lwt_main.run(Xenstore_frontend.create channel a) in
	Old_ring.C_Xenstore.zero b;
	let backend = Lwt_main.run(Xenstore_backend.create channel' (Cstruct.of_bigarray b2)) in
...
let xenstore_hello () =
	...
	with_xenstores
		(fun b1 b2 a b backend ->
			...
			Lwt_main.run(Xenstore_backend.Reader.advance backend ofs);
...
module Console_frontend = Console_ring.Frontend(Inheap_events)
module Console_backend = Console_ring.Backend(Inheap_events)
...
let with_consoles fn =
	...
	let port, channel = Lwt_main.run(Inheap_events.listen 0) in
	let channel' = Lwt_main.run(Inheap_events.connect 0 port) in
	let f = Lwt_main.run(Console_frontend.create channel a) in
	Old_ring.C_Console.zero b;
	let backend = Lwt_main.run(Console_backend.create channel' (Cstruct.of_bigarray b2)) in
...
let console_hello () =
	...
	with_consoles
		(fun b1 b2 a b backend ->
			...
			Lwt_main.run(Console_backend.Reader.advance backend ofs);
			let x = to_string buffers in
